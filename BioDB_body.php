...
class BioDB
{
    ...
    public static function doForExternalTableTemplate($parser, $frame, $args)
    {
        ...
        $extraparams = [ "html" => false, "header" => "", "footer" => "" ];
    ...
    private static function assignTemplateValues($variables)
    {
        ...
        return([ $values, $defaultValues, $fullValues ]);
    ...
    public static function doStoreExternalTable(&$parser)
    {
        ...
        if ($smwgDefaultStore != 'SMWSQLStore3' && ! class_exists('SIOHandler')) {
            ...
            return '<strong class="error">BioDB: Semantic Internal Objects is not installed</strong>';
        }
        ...
        for ($i = 0; $i < $num_loops; $i++) {
            ...
            call_user_func_array([ 'SIOHandler', 'doSetInternal' ], $refParams);
    ...
    public static function callSubobject($parser, $params)
    {
        ...
        if (class_exists('SMW\SubobjectParserFunction')) {
            ...
            call_user_func_array([ 'SMW\SubobjectHandler', 'render' ], $subobjectArgs);
        } elseif (class_exists('SMW\SubobjectParser')) {
            // Old version of SMW 1.9 - can be removed at some point
            call_user_func_array([ 'SMW\SubobjectParser', 'render' ], $subobjectArgs);
        } elseif (class_exists('SMW\Subobject')) {
            // Old version of SMW 1.9 - can be removed at some point
            call_user_func_array([ 'SMW\Subobject', 'render' ], $subobjectArgs);
        } else {
            // SMW 1.8
            call_user_func_array([ 'SMWSubobject', 'render' ], $subobjectArgs);
        }
        return;
    }
     ...
     */
    public static function doClearExternalData(&$parser)
    {
        global $wgBioDBValues;
        $wgBioDBValues = [];
    }
